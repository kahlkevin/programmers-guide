...
objects for quicker future access.

And a `SpriteFrame` is an object that contains the image file name and a `Rect` that
...
### Sprite properties affected by anchor point
Using _Anchor Point_ affects only properties that can be transformed. This includes
